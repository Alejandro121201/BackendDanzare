// routers/productos/productos.dao.js
const { sql, getPool } = require("../../db/mssql");
const Q = require("./utils/productos.query");
...
async function insert(payload) {
  ...
  req.input("talla", sql.NVarChar(20), talla ?? null);
...
async function updateById(id, payload) {
  const { nombre, descripcion, precio, talla, imagen_url, activo } = payload;
  const pool = await getPool();
